        ...
        for file in files:
            with open(file, 'r') as stream:
                try:
                    ...
                    if 'kernel' in data:
                        linuxkityaml['kernel'].update(data['kernel'])
                    if 'init' in data:
                        linuxkityaml[
                            'init'] = linuxkityaml['init'] + data['init']
                    if 'onboot' in data:
                        linuxkityaml[
                            'onboot'] = linuxkityaml['onboot'] + data['onboot']
                    if 'services' in data:
                        linuxkityaml[
                            'services'] = linuxkityaml['services'] + data['services']
                    if 'files' in data:
                        linuxkityaml[
                            'files'] = linuxkityaml['files'] + data['files']
                    if 'trust' in data:
                        linuxkityaml['trust'].update(data['trust'])
                except yaml.YAMLError as exc:
                    print("Failed to read YAML from:", file)
                    print("  >", exc)
    except IOError as e:
...
def buildkernelgraph():
    ...
    graph = add_nodes(digraph(), [('kernel', {'label': colonoscopykey})])
...
def addonbootgraph(gIN):
    ...
    for key, value in onboot.items():
        ...
        if 'binds' in value:
            bindstodict(colonoscopyimagename, value['binds'], dictofbinds)
        ...
        parsemobyconfig(colonoscopyimagename, mobyconfig, dictofbinds)
        ...
        if dictofbinds:
            ...
            for bind, innerdict in dictofbinds.items():
                nodeslist.append((innerdict['dest'], {
                    'label': innerdict['dest'],
                    'color': 'red'
                }))
                edgelist.append((('rootfs', innerdict['dest']), {
                    'label':
                    bind + "\n rshared:" + str(innerdict['rshared']) +
                    "\n rbind:" + str(innerdict['rbind']) + "\n target:" + str(
                        innerdict['target']),
                    'color':
                    'red'
                }))
                edgelist.append(((innerdict['dest'],
                                  key + "\n" + colonoscopyimagename), {
                                      'color': 'red'
                                  }))
        else:
...
def getdockerlabel(imagenametag):
    ...
    mobyprojectconfig = subprocess.getoutput(
        "docker inspect --format='{{json .Config.Labels}}' " + imagenametag)
    return mobyprojectconfig


def parsemobyconfig(colonoscopyimagename, mobyconfig, dictbinds):
    ...
    try:
        ...
        bindstodict(colonoscopyimagename, reparsed_json["binds"], dictbinds)
    except ValueError:
...
def bindstodict(colonoscopyimagename, binds, dictbinds):
    try:
